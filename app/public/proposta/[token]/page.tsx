  ...
  return (
    <PublicFormLayout title="Avaliação de Proposta — V3A">
      <div className="max-w-3xl mx-auto space-y-8">
        {loading ? (
          <div className="bg-white border border-[#E2E3E4] rounded-3xl p-16 text-center text-slate-600 space-y-4 shadow-2xl">
            <div className="inline-block animate-spin w-10 h-10 border-4 border-[#FFCB05] border-t-transparent rounded-full shadow-none" />
            <p className="font-medium text-slate-700 text-lg">Preparando sua proposta oficial...</p>
          </div>
        ) : errorMessage ? (
          <div className="bg-white border border-rose-200 rounded-3xl p-10 text-center space-y-5 shadow-2xl">
            <div className="w-20 h-20 bg-rose-50 border border-rose-200 rounded-full flex items-center justify-center mx-auto text-rose-600 shadow-[0_0_30px_rgba(244,63,94,0.15)]">
              <AlertTriangle size={40} />
            </div>
            <h2 className="text-2xl font-bold text-slate-900">Ops! Não foi possível acessar a proposta</h2>
            <p className="text-slate-600 text-base max-w-md mx-auto">{errorMessage}</p>
          </div>
        ) : submittedSuccess ? (
          <div className="bg-white border border-emerald-200 rounded-3xl p-12 text-center space-y-6 shadow-lg">
            <div className="w-24 h-24 bg-emerald-100 border border-emerald-200 rounded-full flex items-center justify-center mx-auto text-emerald-700 ">
              <CheckCircle2 size={48} />
            </div>
            <h2 className="text-3xl font-black text-slate-900 tracking-tight">Resposta Registrada!</h2>
            ...
            <div className="pt-6 mt-2 border-t border-[var(--v3a-gray-200)] text-[10px] text-slate-600 font-bold tracking-widest uppercase">
          ...
          <div className="space-y-8 text-slate-800">
            {/* Header Badge */}
            <div className="bg-[var(--v3a-gray-100)] border border-[#E2E3E4] rounded-3xl p-8 shadow-2xl relative overflow-hidden">
              <div className="absolute top-0 right-0 px-5 py-2 bg-[#FFCB05] text-black text-[10px] font-black uppercase tracking-widest rounded-bl-2xl shadow-md">
                Job ID: {proposalData.job_code}
              </div>

              <div className="space-y-4">
                <span className="text-[10px] uppercase tracking-widest font-black text-[#FFCB05] bg-[#FFF6D6] border border-[#FFE680] px-3 py-1.5 rounded-full inline-flex">
                  Proposta Oficial
                </span>
                <h1 className="text-3xl md:text-4xl font-black text-slate-900 tracking-tight leading-tight">{proposalData.job_title}</h1>
                <p className="text-base text-slate-600 flex items-center gap-2.5 font-light">
                  <User size={16} className="text-[#FFCB05]" /> Olá, <strong className="text-slate-900 font-bold">{proposalData.freelancer_name}</strong>
                </p>
              </div>
            </div>

            {/* Read-Only Details Card */}
            <div className="bg-white border border-[#E2E3E4] rounded-3xl p-8 space-y-8 shadow-xl">
              <h3 className="text-xs font-black text-[#FFCB05] uppercase tracking-widest border-b border-[var(--v3a-gray-200)] pb-4">
                Visão Geral do Projeto
              </h3>

              <div className="grid grid-cols-1 md:grid-cols-2 gap-5 text-sm">
                <div className="flex items-start gap-4 p-4 rounded-2xl bg-[#F0F0F0] border border-[#E2E3E4]">
                ...
                <div className="flex items-start gap-4 p-4 rounded-2xl bg-[#F0F0F0] border border-[#E2E3E4]">
                  <div className="p-2.5 bg-[#FFF6D6] rounded-xl text-[#FFCB05]">
                ...
                <div className="flex items-start gap-4 p-4 rounded-2xl bg-emerald-50 border border-emerald-200 ">
                  ...
                  <div>
                    <span className="text-[10px] text-emerald-700/70 block uppercase tracking-widest font-black mb-0.5">Valor Base / Remuneração</span>
                    <strong className="text-emerald-700 text-xl block mb-1">{formatCurrency(proposalData.expected_total_value)}</strong>
                    <span className="text-[11px] text-emerald-700 font-semibold uppercase tracking-wider bg-emerald-100 px-2 py-0.5 rounded inline-block">
              ...
              {proposalData.description && (
                <div className="space-y-2 pt-2">
                  ...
                  <div className="p-4 rounded-xl bg-[#F0F0F0] border border-slate-200 text-slate-700 text-sm whitespace-pre-line leading-relaxed">
                    {proposalData.description}
                  </div>
                </div>
              )}
            ...
            <div className="bg-white border border-slate-200 rounded-2xl p-6 space-y-6 shadow-xl">
              ...
              {responseType === 'refused' && (
                <div className="space-y-4 p-4 rounded-xl bg-rose-50 border border-rose-200">
                  <label className="block text-xs font-semibold text-rose-700 uppercase tracking-wider">
                    Motivo da Recusa *
                  </label>
                  <select
                    className="w-full bg-white border border-slate-200 rounded-lg p-2.5 text-sm text-slate-700 focus:outline-none focus:border-rose-500"
                    value={refusalReason}
                  ...
                  <div>
                    ...
                    <textarea
                      className="w-full bg-white border border-slate-200 rounded-lg p-2.5 text-sm text-slate-700 focus:outline-none focus:border-slate-600"
                      rows={2}
                  ...
                  <div className="flex gap-3 pt-2">
                    <button
                      ...
                      className="flex-1 py-2.5 px-4 bg-rose-600 hover:bg-rose-500 text-white font-medium rounded-xl text-sm transition-all flex items-center justify-center gap-2 shadow-lg shadow-rose-200"
                    >
              ...
              {responseType === 'accepted_with_reservations' && (
                <div className="space-y-4 p-4 rounded-xl bg-amber-50 border border-amber-200">
                  <label className="block text-xs font-semibold text-amber-800 uppercase tracking-wider">
                    Descreva suas ressalvas de valores ou condições *
                  </label>
                  <textarea
                    className="w-full bg-white border border-slate-200 rounded-lg p-3 text-sm text-slate-700 focus:outline-none focus:border-amber-500"
                    rows={3}
                  ...
                  <div className="flex gap-3 pt-2">
                    <button
                      ...
                      className="flex-1 py-2.5 px-4 bg-amber-600 hover:bg-amber-500 text-white font-medium rounded-xl text-sm transition-all flex items-center justify-center gap-2 shadow-lg shadow-amber-200 disabled:opacity-50"
                    >
              ...
              {responseType === null && (
                <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                  <button
                    ...
                    className="py-3 px-4 bg-emerald-600 hover:bg-emerald-500 text-white font-semibold rounded-xl text-sm transition-all flex items-center justify-center gap-2 shadow-lg shadow-emerald-200"
                  >
                  ...
                  <button
                    ...
                    className="py-3 px-4 bg-amber-600 hover:bg-amber-500 text-white font-semibold rounded-xl text-sm transition-all flex items-center justify-center gap-2 shadow-lg shadow-amber-200"
                  >
                  ...
                  <button
                    ...
                    className="py-3 px-4 bg-rose-600 hover:bg-rose-500 text-white font-semibold rounded-xl text-sm transition-all flex items-center justify-center gap-2 shadow-lg shadow-rose-200"
                  >
